...
export function AdminLogin({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="relative flex min-h-screen overflow-hidden bg-bg">
      ...
      <aside className="relative hidden flex-1 items-end overflow-hidden lg:flex" aria-hidden="true">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_10%,color-mix(in_srgb,var(--color-accent)_28%,transparent),transparent_55%),radial-gradient(ellipse_at_85%_75%,color-mix(in_srgb,var(--color-accent-hover)_18%,transparent),transparent_50%)]" />
        <div className="absolute inset-0 opacity-[0.35] [background-image:linear-gradient(var(--color-border)_1px,transparent_1px),linear-gradient(90deg,var(--color-border)_1px,transparent_1px)] [background-size:56px_56px] [mask-image:radial-gradient(ellipse_at_center,black,transparent_75%)]" />
        <motion.div animate={{ y: [0, -18, 0] }} transition={{ duration: 9, repeat: Infinity, ease: 'easeInOut' }} className="absolute left-[12%] top-[16%] h-64 w-64 rounded-full bg-accent/20 blur-3xl" />
        <motion.div animate={{ y: [0, 22, 0] }} transition={{ duration: 11, repeat: Infinity, ease: 'easeInOut' }} className="absolute bottom-[10%] right-[8%] h-72 w-72 rounded-full bg-accent-hover/15 blur-3xl" />

        <svg viewBox="0 0 800 400" preserveAspectRatio="none" className="absolute inset-x-0 bottom-0 h-[46%] w-full">
          <defs>
          ...
          <path d="M0 330 C90 310 130 340 210 290 S340 260 410 220 S540 180 610 120 S730 70 800 30 V400 H0Z" fill="url(#vault-area)" />
          <motion.path
            d="M0 330 C90 310 130 340 210 290 S340 260 410 220 S540 180 610 120 S730 70 800 30"
            fill="none"
        ...
        <div className="relative z-10 max-w-xl p-14 pb-20">
          ...
          <h2 className="mt-5 font-display text-4xl font-semibold leading-tight text-text xl:text-5xl">
            Your whole life,
            <br />
            <span className="bg-gradient-to-r from-accent to-accent-hover bg-clip-text text-transparent">beautifully organised.</span>
          </h2>
          <p className="mt-4 max-w-md text-sm leading-relaxed text-text-secondary">Documents, money, investments and health records in one encrypted place, with an assistant that answers only from your own data.</p>
      ...
      {/* Form side */}
      <main className="relative flex w-full items-center justify-center px-6 py-12 lg:w-[480px] lg:shrink-0 lg:border-l lg:border-border xl:w-[540px]">
        <div className="pointer-events-none absolute inset-0 lg:hidden [background:radial-gradient(ellipse_at_50%_0%,color-mix(in_srgb,var(--color-accent)_25%,transparent),transparent_60%)]" />
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: 'easeOut' }} className="relative w-full max-w-sm">
