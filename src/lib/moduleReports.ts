    ...
    sections: [
      {
        title: 'Summary',
        kpis: [
          { label: 'Net P&L', value: formatSignedInr(a.net), tone: a.net >= 0 ? 'good' : 'bad', note: 'after est. charges' },
          { label: 'Gross P&L', value: formatSignedInr(a.gross), note: `${a.trades} closed trades` },
          { label: 'Charges (est.)', value: inr(a.charges), tone: 'warn' },
      ...
      { title: 'Charges (estimated)', bars: Object.entries(a.chargeParts).map(([k, v]) => ({ label: k, value: v, display: inr(v, 2) })) },
